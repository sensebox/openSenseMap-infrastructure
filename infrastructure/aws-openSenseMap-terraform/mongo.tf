...
resource "aws_instance" "mongo_host_v2" {
  count = "${var.mongo_enabled ? 3 : 0}"

  ami                    = "${lookup(var.ami_v2, var.aws_region)}"
  instance_type          = "${var.mongo_instance_type_v2}"
  subnet_id              = "${element(data.aws_subnet_ids.opensensemap_subnets.ids, count.index % 2)}"
  vpc_security_group_ids = ["${aws_security_group.allow_all_outbound.id}", "${aws_security_group.ssh_unims.id}", "${aws_security_group.mongo_internal.id}"]
  key_name               = "${var.aws_key_name}"
  ...
  ebs_block_device {
    volume_type           = "${var.mongo_volume_v2["volume_type"]}"
    volume_size           = "${var.mongo_volume_v2["volume_size"]}"
    iops                  = "${var.mongo_volume_v2["iops"]}"
    delete_on_termination = false
    device_name           = "/dev/xvdb"
  }

  tags {
    Name    = "mongoDB_v2_${count.index}"
    Project = "openSenseMap"
  }
}
