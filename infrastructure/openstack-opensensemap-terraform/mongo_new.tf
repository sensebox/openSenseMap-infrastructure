resource "openstack_compute_instance_v2" "mongo_host_tb" {
  ...
  security_groups = ["${openstack_compute_secgroup_v2.mongo_internal_tb.name}", "${openstack_compute_secgroup_v2.ssh_from_bastion.name}", "${openstack_compute_secgroup_v2.prometheus_internal.name}"]
